 ...
 * الإصدار المحدث: يعتمد كلياً على الاتصال المركزي الموحد (window.teraSupabase)
 */
...
// الدالة النهائية والوحيدة المسؤولة عن معالجة طلبات التسجيل عبر السيرفر والاتصال المركزي
async function submitForm() {
    if (validateStage1Logic() && validateStage2Logic()) {
        const submitBtn = document.getElementById("action-submit-btn");
        const originalText = submitBtn.innerHTML;

        // استدعاء اتصال كائن العميل الموحد من السكربت المركزي المركب نافذته سابقاً
        const db = window.teraSupabase;

        if (!db) {
            alert("❌ تعذر الاتصال بقاعدة البيانات. يرجى مراجعة الترتيب الصحيح لتحميل الملفات بصفحة الـ HTML وضمان تحميل ملف supabase-client.js أولاً.");
            return;
        }
        ...
        try {
            // تنفيذ الطلب مباشرة عبر المحرك الموحد المركزي
            ...
            if (error) {
                console.error("Supabase Central Client Error:", error.message);
            ...
            console.error("Central Network Connection Error:", err);
            alert("❌ تعذر الاتصال بالخادم المركزي. يرجى التحقق من جودة الاتصال بالشبكة والمحاولة لاحقاً.");
            submitBtn.innerHTML = originalText;
            submitBtn.removeAttribute("disabled");
        }
    } else {
