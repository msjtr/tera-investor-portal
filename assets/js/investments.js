...
(function() {
    ...
    function initInvestmentDetails() {
        ...
        if (btnJoin) { btnJoin.href = "cancelled-investments.html?id=" + opp.id; btnJoin.style.display = 'inline-flex'; }
    }
    ...
    function initCancelledInvestments() {
        ...
        window.executeCalculations = function() {
            ...
            document.getElementById('resToPayNow').innerText = window.formatMoneySafe(capital * 1.15) + " ر.س";
        };
    }
    ...
    window.processPayment = function() { alert("تم الدفع بنجاح!"); window.location.href = "../dashboard/index.html"; };

    function initInvestments() {
        const path = window.location.pathname.toLowerCase();
        if (path.includes('completed-investments')) initInvestmentDetails();
        if (path.includes('cancelled-investments')) initCancelledInvestments();
    }
