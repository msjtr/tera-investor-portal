/**
 * dashboard.js – v10.3 (الإصدار النهائي المتكامل)
 * يعمل بتناغم مع #alertsContainer داخل #alertsPanel
 */
...
(function() {
    ...
    // ============================================================
    // 6. تحميل وعرض التنبيهات (متوافقة مع #alertsContainer)
    // ============================================================
    ...
    function renderAlerts(alerts, container) {
        ...
        // الحصول على الحاوية الداخلية (موجودة في HTML أو ننشئها إن لزم)
    ...
    function renderIncompleteProfile(panel, req) {
        ...
        stages.forEach(stage => {
            ...
            <div class="stage-item ${done ? 'completed' : 'pending'}">
                ${stage.link ? `<a href="${stage.link}">` : ''}
                    ...
                    <div class="stage-status">${done ? '✔ مكتمل' : '⏳ مطلوب'}</div>
                ${stage.link ? '</a>' : ''}
            </div>`;
        });
        ...
        if (stages.some(s => !req?.[s.key])) {
            ...
            html += `<div class="alert-item-box alert-success" style="margin-top:12px;">تم استلام طلبكم بنجاح.</div>`;
        }
        html += `</div>`;
