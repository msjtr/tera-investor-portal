...
(function() {
    ...
    /**
     * ترجمة حالة طلب التحقق (وليس حالة الجلسة)
     */
    ...
    function renderIncompleteProfile(panel, req) {
        ...
        if (stages.some(s => !req?.[s.key])) {
            ...
            html += `<div class="alert-item-box alert-success" style="margin-top:12px;">
                <i class="fas fa-check-circle"></i> تم استلام طلبكم بنجاح، وسيتم تحويله للمراجعة.
            </div>`;
        }
        html += `</div>`;
    ...
    function renderRequestStatus(panel, req) {
        ...
        // استخدام دالة الترجمة المحلية (وليس UIHelpers المخصصة للجلسات)
