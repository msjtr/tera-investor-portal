...
function exportCsv() {
  let csv = '﻿' + 'رقم المرجع,البنك,الآيبان,المبلغ,تاريخ الطلب,الحالة
';
  __allRequests.forEach(function(r) {
    csv += '"' + (r.reference_number||'') + '","' + (r.bank_name||'') + '","' + (r.iban||'') + '","' + (r.amount||0) + '","' + fmtDateShort(r.requested_at||r.created_at) + '","' + (STATUS_LABELS[r.status]||r.status) + '"
';
  });
